...
8. [Что дальше](#8-что-дальше) — недостающий API, который использует реальный код, по порядку.
...
Состояние на версию **0.1.0-preview.5** (сентябрь 2026).
...
- **`DataGridView`**: необработанный `DataError` не показывается окном сообщения (WinForms показывает); у привязанной
  сетки пока нет строки для новых записей (в WinForms есть, когда список разрешает добавление); назначенный `DefaultCellStyle` с
