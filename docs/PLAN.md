...
147. **Строка для новых записей — настоящая строка; первая ячейка становится текущей.** (2026-09-25, Linux. Сессия
    начала работу, не подтянув `main`, и повторила решения 140–142 по-своему; повтор отброшен, перенесено только то,
    чего в `main` не было.) Раньше `NewRowIndex` считал новой последнюю строку, добавленную пользователем, а самой
    строки не было — `Rows.Count` расходился с WinForms на единицу, и типичный код (`Rows.Count - 1`,
    `foreach … if (!row.IsNewRow)`) вёл себя иначе. Теперь, как в WinForms: у непривязанной сетки со столбцами и
    ...
    первую ячейку (не новой строки) — при создании (`OnCreateControl`) и при первых строках, как
    `MakeFirstDisplayedCellCurrentCell`; прежняя осознанная разница («первая ячейка не становится текущей») снята. Из-за
    этого три теста щёлкали по уже текущей ячейке — а щелчок по текущей ячейке начинает правку (`DataGridViewTextBoxCell.
    OnMouseClick`), и переход с неё валидирует её: тесты переписаны (фокус вместо щелчка, счётчик после перехода).
    Свойства источника типа `Image` и то, что `ImageConverter` превращает в картинку (`byte[]`), получают
    `DataGridViewImageColumn`. Не сделано (осознанная разница в compatibility.md): новая строка у привязанной сетки
    (`IBindingList.AllowNew` → `AddNew`/`CancelNew`/`EndNew`), отмена новой строки вторым Esc. Попутно: `Rows.Add(36)` —
    это `Add(int count)`, 36 пустых строк (в WinForms так же), значение — `Rows.Add((object)36)`; тест
    `RowsAddOfOneNumberAddsThatManyRows`. Проверено: `DataGridViewNewRowTests`, оракул `exact/dgv-newrow/*`.

148. **Четвёртый выпуск — `0.1.0-preview.4`.** Заказчик попросил записать сделанное в документацию и выпустить.
    Вошло всё после `0.1.0-preview.3`: решение 146 (порядок работ по использованию, типы на своих местах —
    `TextureBrush`, `FlushIntention`, `TableLayoutControlCollection`, `ImageList.Images.Keys`) и решение 147 (строка для
    новых записей, первая текущая ячейка, столбцы-картинки). Версия поднята везде, как в решениях 143 и 145
    (`Directory.Build.props`, шаблон `netforms-app`, `netformsVersion` расширения, README, docs en/ru, README пакета,
    главные страницы сайта); расширение — `0.1.3` (сетка на холсте показывает строку для новых записей, как дизайнер
    VS). Разделы «Unreleased» обоих журналов изменений стали разделами выпуска. **Меняется поведение для кода:**
    `Rows.Count` непривязанной сетки с `AllowUserToAddRows` теперь на единицу больше (как в WinForms) — это записано в
    CHANGELOG. Выпуск — пуш смены версии в `main` после зелёного CI (оракул `exact/dgv-newrow/*` сошёлся с WinForms на
    коммите 0d86402).

149. **Дизайнер: коллекции компонентов и переключение вкладок.** Заказчик: в свойстве TabPages (и Items у MenuStrip)
    OK редактора падал с «Unable to cast object of type 'System.String' to type 'System.Windows.Forms.TabPage'» —
    редактор коллекции отдавал строки, а хост клал их прямо в `TabPages`/`Items`; страницы показывались как
    `TabPage: {Приход}`; вкладки на холсте не переключались, и раскладывать контролы можно было только на первой.
    Сделано: коллекции компонентов (`TabPageCollection`, `ToolStripItemCollection`, `ListView.ColumnHeaderCollection`,
    `DataGridViewColumnCollection`) идут в инспектор форматом `components` — подписи элементов (`Text`, `HeaderText`,
    `-` у разделителя) и рядом `ItemIds` (имя компонента или `#индекс` у безымянного); `setItems` принимает `ids` —
    какой элемент стоит за строкой. Оставшиеся сохраняют детей и переставляются, выброшенные удаляются со всем
    содержимым (как `remove`), новые создаются как в VS (`InitializeNew`, имя `tabPage3`, у полосы — `ToolStripMenuItem`/
    `ToolStripButton`/`ToolStripStatusLabel`, `-` — `ToolStripSeparator`); освободившееся имя переиспользуется, как в VS.
    Без `ids` строки сопоставляются по подписи. В веб-вью — список с полями, ↑ ↓ ✕, «Добавить». `DesignerViewItem`
    TabControl несёт прямоугольники заголовков (`Tabs`) и `SelectedIndex`; щелчок по заголовку выделяет TabControl и
    пишет `SelectedIndex` — как VS (он тоже сериализует выбранную вкладку); щелчок в панели элементов при выделенном
    TabControl кладёт контрол на показанную страницу. Проверено: `DesignerHostTests` (+3), UI-тест в Chromium
    «tab pages switch with a click…».

150. **Пятый выпуск — `0.1.0-preview.5`.** Заказчик попросил выпустить. Вошло решение 149 (только дизайнер: библиотека
    NetForms не менялась, версия поднята, потому что расширение выходит тем же конвейером по тегу `v<Version>`).
    Версия поднята везде, как в решениях 143, 145 и 148; расширение — `0.1.4`. Разделы «Unreleased» журналов стали
    разделами выпуска.

151. **Дизайнер: буфер обмена, контекстное меню, шаблоны форм, стартовая форма, публикация, обновления.** Заказчик
    выбрал из списка «мелочей» пять пунктов и добавил седьмой — обновления, включая инструменты. Копирование:
    запрос хоста `copy` отдаёт JSON (`netforms/components-1`): тип, свойства по тем же правилам `ShouldSerialize`, что
    у writer, ссылки на компоненты по имени, дети, части (`SplitContainer.Panel1`), коллекции компонентов, списки в
    форматах редактора коллекций, стили и ячейки `TableLayoutPanel`, обработчики. Операции `paste` (в выделенное:
    контейнер, контейнер выделенного контрола, показанная вкладка `TabControl`; пункты — в полосу или меню) и
    `duplicate` (рядом с оригиналом). Имя сохраняется, если свободно (как VS при вставке в другую форму), обработчики —
    только при вставке в ту же форму; вставленное сдвигается на 8 px, пока лежит точно на существующем контроле;
    `TabIndex` — следующий в контейнере. Картинки не копируются (нет записи `.resx`). Контекстное меню холста —
    как в VS. Шаблоны `netforms-dialog`, `-aboutbox`, `-login`, `-splash` сгенерированы самим хостом (канонический
    вид); подписи формы входа — фиксированного размера, а высоту однострочного `TextBox` (шрифт + 7) тест шаблонов
    подставляет с платформы, как тест HelloForms — размер подписи. Попутно баг: `AcceptButton`/`CancelButton` имеют
    тип `IButtonControl` (не `IComponent`) — сетка показывала текстовое поле, а `ReferenceConverter` без контекста
    молча давал `null`; теперь это ссылка на компонент со списком кнопок. «Сделать стартовой формой» правит
    `Application.Run(new …())` (имя — как его разрешит C# из пространства имён `Program`). «Опубликовать» —
    `dotnet publish -r <rid> --self-contained true|false` в `publish/<rid>` (одним файлом — нет, см. install.md).
    «Проверить обновления» — NuGet flatcontainer (новейший NetForms), GitHub Releases (`.vsix` своей платформы,
    ставится `workbench.extensions.installExtension`), `dotnet --list-sdks` против `latest-sdk` из release metadata;
    SDK не обновляется сам — команда (winget/apt/dnf/dotnet-install.sh) вписывается в терминал без запуска (нужен
    `sudo`). Шаблоны новее версии расширения теперь принимаются. Проверено: `DesignerHostTests` (+4 на буфер),
    `TemplateTests` (+4 шаблона, сборка из пакета), `test/project.test.js`, UI-тест копирования и контекстного меню.

152. **Шестой выпуск — `0.1.0-preview.6`.** Заказчик попросил выпустить. Вошли решение 151 (буфер обмена, контекстное
    меню, шаблоны `netforms-dialog`/`-aboutbox`/`-login`/`-splash`, стартовая форма, публикация, проверка обновлений,
    `AcceptButton`/`CancelButton` в сетке). Библиотека NetForms не менялась с `preview.4`; новые шаблоны едут в
...
162. **Выпуск `0.1.0-preview.7`, расширение 0.1.6; фикстура — в решении.** Первый CI после решений 158–161 упал на
    обеих ОС: 10 из 20 `ControlLibraryTests` — «фикстура не собрана». Причина: CI собирает `NetForms.slnx -c Release`,
    а проект вне решения, на который ссылается тестовый (`ReferenceOutputAssembly=false`), MSBuild собирает в
    конфигурации **по умолчанию** (Debug) — в `bin/Release` его не было. Локально проходило, потому что собиралось в
    Debug. Исправление: `tests/Fixtures/ControlLibrary` добавлена в `NetForms.slnx` (получает конфигурацию решения;
    `IsPackable=false`, в пакеты не попадает), а тест берёт сборку фикстуры в любой конфигурации, своей — первой.
    Проверено воспроизведением: чистая `dotnet build NetForms.slnx -c Release` — 504/504. Выпуск — как прежние
    (решение 152): версия в `Directory.Build.props`, шаблоне, `netformsVersion` и версии расширения, README, документации
    (англ., рус.), README пакета и главных страницах сайта; цифры состояния в README обновлены (API 747/144/363, тесты
...
> Прочитай `CLAUDE.md`, в `docs/PLAN.md` — раздел «С чего начать следующую сессию» и решения 127–152. Сначала
> `git pull` — в `main` могут быть коммиты других сессий (решение 147). Работай сразу в `main` (разрешено заказчиком). Пункты 1–2 сделаны (решения 139–146); дальше пункт 3 — по списку «What comes next»
...
использованию и типы на своих местах (решение 146). До него — `0.1.0-preview.3` (решение 145): инспектор дизайнера не сбрасывается при правке
