...
> Прочитай `CLAUDE.md`, в `docs/PLAN.md` — раздел «С чего начать следующую сессию» и решения 127–151. Сначала
...
> `designer: npm test`. Решения, которых нет в плане, записывай в журнал (со 152).

**Состояние на 2026-09-25.** Выпущен `0.1.0-preview.5` (решение 150): редактор коллекций TabPages и пунктов меню,
переключение вкладок на холсте дизайнера (решение 149). До него — `0.1.0-preview.4` (решение 148): строка для новых записей `DataGridView` —
